...
from pipecat.frames.frames import AudioRawFrame, StartFrame, TextFrame
...
async def main():
    try:
        ...
        stt = OpenAISTTService(api_key='dummy')
        llm = GoogleLLMService(api_key=os.getenv('GEMINI_API_KEY', 'AIzaSyB4G3GygDMCwC-SZ1EtXdSnr4Dn0fP4_Q0'), model='gemini-2.5-flash')
        tts = ElevenLabsTTSService(api_key=os.getenv('ELEVENLABS_API_KEY', 'sk_1efd713557745a1b5c15bb8389e95d51023600689bc874a1'), voice_id='EXAVITQu4vr4xnSDxMaL')

        messages = [
            {'role': 'system', 'content': 'You are a helpful conversational AI assistant inside a video avatar. You are a distinguished Omani gentleman. Respond warmly and concisely in English or Arabic. Do NOT use emojis.'},
        ]
        ...
        async def on_first_participant_joined(transport, participant):
            await task.queue_frames([TextFrame('Hello there! How can I assist you today?')])

        runner = PipelineRunner()
        
        logger.info(f'Starting AUTONOMOUS WebRTC streaming bot in {room_name}...')
